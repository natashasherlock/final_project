   ...
   // (void)Width_out; // silence declared but never referenced warning. remove this line when you start working
    ...
    for (int c = 0; c < Channel; c++) { // sum over all input channels
        for (int p = 0; p < K; p++){ // loop over KxK filter
            //for (int q = 0; q < K; q++)
                acc += in_4d(z, c, h + p, w + 0) * mask_4d(m, c, p, 0);
                acc += in_4d(z, c, h + p, w + 1) * mask_4d(m, c, p, 1);
                acc += in_4d(z, c, h + p, w + 2) * mask_4d(m, c, p, 2);
                acc += in_4d(z, c, h + p, w + 3) * mask_4d(m, c, p, 3);
                acc += in_4d(z, c, h + p, w + 4) * mask_4d(m, c, p, 4);
                acc += in_4d(z, c, h + p, w + 5) * mask_4d(m, c, p, 5);
                acc += in_4d(z, c, h + p, w + 6) * mask_4d(m, c, p, 6);
        }
    }
